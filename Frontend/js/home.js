...
// Check for the token at the very beginning
const token = localStorage.getItem("token");
let socket = null;

if (!token) {
    // If no token, redirect to login and stop execution
    window.location.href = "./login.html";
} else {
    // If token exists, initialize the socket connection
...
if (socket) {
    socket.on("new_global_message", (message) => {
        // console.log("Received a new global message:", message); 
...
chatForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const message = chatInput_group.value.trim();message
    console.log(message);
if (!message || !currentGroupId) return;

    // Change this section to use Socket.IO
    if (socket && socket.connected) {
        ...
        // Optional: Immediately append the message for the sender's UI
        // This provides instant feedback without waiting for the broadcast.
        const user = { name: "You" }; 
        appendGroupMessage({ User: user, content: message });
    } else {
        alert("Socket.IO not connected. Please try again.");
    }
...
async function openChat(groupId, groupName) {
    try {
        ...
        // Emit 'join_group' event to the server to join the specific group's room
        if (socket) {
...
// Helper function to append a message to the global chat area
function appendGlobalMessage(message) {
    ...
    // console.log(message);
    msgDiv.innerHTML = `<strong>${message.User.name}:</strong> ${message.content}`;
    chatArea.appendChild(msgDiv);
    chatArea.scrollTop = chatArea.scrollHeight;
}

// Helper function to append a message to the group chat modal
function appendGroupMessage(message) {
    const msgDiv = document.createElement("div");
    msgDiv.className = "p-2 rounded bg-gray-200 text-black";
    msgDiv.innerHTML = `<strong>${message.User.name}:</strong> ${message.content}`;
...
sendBtn.addEventListener("click", async () => {
    ...
    if (socket && socket.connected) {
        // Step 1: Emit the message to the backend via Socket.IO
        socket.emit("new_global_message", { message: message });
        
        // Step 2: Manually append the message to the sender's UI for instant feedback
        // const user = { name: "You" }; // Or fetch your own user name
        appendGlobalMessage({ User: user, content: message });
        
        chatInput.value = "";
    } else {
        alert("Chat service is not connected. Please refresh the page.");
    }
});
...
//     auth: { token: localStorage.getItem("token") }, // send token to backend
// });
